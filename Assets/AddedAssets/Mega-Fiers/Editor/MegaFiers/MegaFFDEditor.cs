				...
				for ( int z = 0; z < gs; z++ )
				{
					for ( int y = 0; y < gs; y++ )
					{
						for ( int x = 0; x < gs; x++ )
						{
							...
							{
								var fmh_140_42_638176027079164677 = Quaternion.identity; ttp = Handles.FreeMoveHandle(tp,Quaternion.identity, ffd.KnotSize * 0.1f, Vector3.zero, Handles.CircleHandleCap);
